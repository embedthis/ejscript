# Ejscript References

**Project**: Ejscript (Ejscript for Bun)
**Last Updated**: 2025-10-17

## Official Documentation

### Ejscript
- **Official Site**: https://www.embedthis.com/ejscript/
- **Documentation**: https://www.embedthis.com/ejscript/doc/
- **API Reference**: https://www.embedthis.com/ejscript/doc/api/ejscript/
- **GitHub**: https://github.com/embedthis/ejscript (archived)

### Bun
- **Official Site**: https://bun.sh/
- **Documentation**: https://bun.sh/docs
- **API Reference**: https://bun.sh/docs/api
- **Runtime APIs**: https://bun.sh/docs/runtime/bun-apis
- **GitHub**: https://github.com/oven-sh/bun
- **Discord**: https://bun.sh/discord

### TypeScript
- **Official Site**: https://www.typescriptlang.org/
- **Handbook**: https://www.typescriptlang.org/docs/handbook/
- **Type Reference**: https://www.typescriptlang.org/docs/handbook/2/everyday-types.html
- **Module Resolution**: https://www.typescriptlang.org/docs/handbook/module-resolution.html
- **GitHub**: https://github.com/microsoft/TypeScript

## Technical References

### JavaScript/ECMAScript
- **MDN Web Docs**: https://developer.mozilla.org/en-US/docs/Web/JavaScript
- **ECMAScript Specification**: https://tc39.es/ecma262/
- **ES Modules**: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules
- **Prototype Inheritance**: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Inheritance_and_the_prototype_chain

### Node.js APIs (used by Bun)
- **File System (fs)**: https://nodejs.org/api/fs.html
- **Path**: https://nodejs.org/api/path.html
- **Child Process**: https://nodejs.org/api/child_process.html
- **Crypto**: https://nodejs.org/api/crypto.html
- **URL**: https://nodejs.org/api/url.html

### HTTP/Networking
- **Fetch API**: https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API
- **HTTP Status Codes**: https://developer.mozilla.org/en-US/docs/Web/HTTP/Status
- **HTTP Headers**: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers
- **WebSocket**: https://developer.mozilla.org/en-US/docs/Web/API/WebSocket
- **MIME Types**: https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types

## Design Patterns & Best Practices

### Object-Oriented Design
- **Singleton Pattern**: https://refactoring.guru/design-patterns/singleton
- **Factory Pattern**: https://refactoring.guru/design-patterns/factory-method
- **Observer Pattern**: https://refactoring.guru/design-patterns/observer
- **Strategy Pattern**: https://refactoring.guru/design-patterns/strategy

### TypeScript Best Practices
- **Do's and Don'ts**: https://www.typescriptlang.org/docs/handbook/declaration-files/do-s-and-don-ts.html
- **Type Guards**: https://www.typescriptlang.org/docs/handbook/2/narrowing.html
- **Generics**: https://www.typescriptlang.org/docs/handbook/2/generics.html
- **Utility Types**: https://www.typescriptlang.org/docs/handbook/utility-types.html

### Testing
- **Bun Test Runner**: https://bun.sh/docs/cli/test
- **Testing Best Practices**: https://testingjavascript.com/
- **Jest API Reference**: https://jestjs.io/docs/api (similar to Bun test API)

## Related Projects

### Similar Projects
- **Node.js**: https://nodejs.org/ - Server-side JavaScript runtime
- **Deno**: https://deno.land/ - Modern JavaScript/TypeScript runtime
- **QuickJS**: https://bellard.org/quickjs/ - Small JavaScript engine

### Ejscript Ecosystem
- **Appweb**: https://www.embedthis.com/appweb/ - Embedded web server
- **GoAhead**: https://www.embedthis.com/goahead/ - Embedded web server
- **MakeMe**: https://www.embedthis.com/makeme/ - Build tool

## Tools & Utilities

### Development Tools
- **VS Code**: https://code.visualstudio.com/
- **TypeScript Language Server**: https://github.com/typescript-language-server/typescript-language-server
- **ESLint**: https://eslint.org/
- **Prettier**: https://prettier.io/

### Build & Package Management
- **Bun Package Manager**: https://bun.sh/docs/cli/install
- **npm**: https://www.npmjs.com/
- **Package.json**: https://docs.npmjs.com/cli/v9/configuring-npm/package-json

### Version Control
- **Git**: https://git-scm.com/doc
- **Semantic Versioning**: https://semver.org/
- **Keep a Changelog**: https://keepachangelog.com/

## Standards & Specifications

### File Formats
- **JSON**: https://www.json.org/
- **JSON5**: https://json5.org/
- **YAML**: https://yaml.org/
- **Markdown**: https://commonmark.org/

### Character Encoding
- **UTF-8**: https://en.wikipedia.org/wiki/UTF-8
- **Character Encodings**: https://www.w3.org/International/questions/qa-what-is-encoding

### MIME Types
- **IANA Media Types**: https://www.iana.org/assignments/media-types/media-types.xhtml
- **Common MIME Types**: https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types/Common_types

## Performance & Optimization

### JavaScript Performance
- **V8 Performance**: https://v8.dev/blog
- **JavaScript Performance Tips**: https://developer.mozilla.org/en-US/docs/Web/Performance
- **Memory Management**: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Memory_Management

### Profiling
- **Bun Profiling**: https://bun.sh/docs/runtime/debugging
- **Chrome DevTools**: https://developer.chrome.com/docs/devtools/
- **Performance API**: https://developer.mozilla.org/en-US/docs/Web/API/Performance

## Security

### Best Practices
- **OWASP Top 10**: https://owasp.org/www-project-top-ten/
- **Node.js Security**: https://nodejs.org/en/docs/guides/security/
- **npm Security**: https://docs.npmjs.com/about-security-audits

### Cryptography
- **Web Crypto API**: https://developer.mozilla.org/en-US/docs/Web/API/Web_Crypto_API
- **Crypto Best Practices**: https://cryptobook.nakov.com/

## Community Resources

### Forums & Discussion
- **Bun Discord**: https://bun.sh/discord
- **Stack Overflow**: https://stackoverflow.com/questions/tagged/bun
- **Reddit r/Bun**: https://www.reddit.com/r/Bun/

### Learning Resources
- **JavaScript.info**: https://javascript.info/
- **TypeScript Deep Dive**: https://basarat.gitbook.io/typescript/
- **Node.js Best Practices**: https://github.com/goldbergyoni/nodebestpractices

## Books

### JavaScript
- "JavaScript: The Good Parts" by Douglas Crockford
- "You Don't Know JS" series by Kyle Simpson
- "Eloquent JavaScript" by Marijn Haverbeke (free online)

### TypeScript
- "Programming TypeScript" by Boris Cherny
- "Effective TypeScript" by Dan Vanderkam

### Design Patterns
- "Design Patterns: Elements of Reusable Object-Oriented Software" by Gang of Four
- "Refactoring: Improving the Design of Existing Code" by Martin Fowler

## Project-Specific References

### Internal Documentation
- [README.md](../../README.md) - Project overview
- [CLAUDE.md](../../CLAUDE.md) - AI assistant guidance
- [DESIGN.md](../designs/DESIGN.md) - Architecture and design
- [PLAN.md](../plans/PLAN.md) - Project roadmap
- [PROCEDURES.md](../procedures/PROCEDURES.md) - Development procedures
- [CHANGELOG.md](../logs/CHANGELOG.md) - Change history

### Implementation Notes
- [IMPLEMENTATION.md](../../IMPLEMENTATION.md) - Implementation details
- [PROJECT_SUMMARY.md](../../PROJECT_SUMMARY.md) - Complete feature overview
- [STATUS.md](../../STATUS.md) - Current status
- [QUICK_START.md](../../QUICK_START.md) - Quick start guide

## External Code References

### Inspiration & Reference Implementations
- **Ejscript Source**: Original Ejscript implementation (archived)
- **Node.js Source**: https://github.com/nodejs/node
- **Bun Source**: https://github.com/oven-sh/bun

### Example Projects
- **Bun Examples**: https://github.com/oven-sh/bun/tree/main/examples
- **TypeScript Examples**: https://github.com/microsoft/TypeScript/wiki/Performance

## Utilities & Libraries (for reference)

### Testing
- **Bun Test**: Built-in test runner
- **Jest**: https://jestjs.io/ (similar API)
- **Vitest**: https://vitest.dev/ (Vite-powered)

### Logging
- **pino**: https://github.com/pinojs/pino
- **winston**: https://github.com/winstonjs/winston

### HTTP Clients (for comparison)
- **axios**: https://axios-http.com/
- **node-fetch**: https://github.com/node-fetch/node-fetch

### String Manipulation
- **inflection**: https://github.com/dreamerslab/node.inflection

## Troubleshooting Resources

### Common Issues
- **Bun Issues**: https://github.com/oven-sh/bun/issues
- **TypeScript Issues**: https://github.com/microsoft/TypeScript/issues
- **Stack Overflow**: https://stackoverflow.com/

### Debugging
- **Bun Debugging**: https://bun.sh/docs/runtime/debugging
- **VS Code Debugging**: https://code.visualstudio.com/docs/editor/debugging
- **Chrome DevTools**: https://developer.chrome.com/docs/devtools/

## Contributing

### Guidelines
- **Semantic Versioning**: https://semver.org/
- **Keep a Changelog**: https://keepachangelog.com/
- **Conventional Commits**: https://www.conventionalcommits.org/

### Code Quality
- **Clean Code**: Principles from Robert C. Martin
- **Code Review Best Practices**: https://google.github.io/eng-practices/review/

## License & Legal

### Licensing
- **Ejscript License**: See original Ejscript project
- **MIT License**: https://opensource.org/licenses/MIT
- **Apache 2.0**: https://www.apache.org/licenses/LICENSE-2.0

### Copyright
- **Embedthis Software**: https://www.embedthis.com/
- **Copyright Best Practices**: https://www.copyright.gov/

## Updates & Maintenance

This references document should be updated when:
- New external resources are discovered
- Links become outdated or broken
- New project-related documentation is created
- New tools or utilities are adopted

---

**Last Reviewed**: 2025-10-17
**Next Review**: 2025-11-17 (monthly)
