...
static void generateImages(EjsMod *mp)
{
    ...
    for (df = docFiles; df->path; df++) {
        ...
        if (file == 0) {
            mprLog("ejs doc", 0, "Cannot create %s", path);
            mp->errorCount++;
            return;
        }
        if (mprWriteFile(file, df->data, df->size) != df->size) {
            mprLog("ejs doc", 0, "Cannot write to buffer");
            mp->errorCount++;
            return;
        }
        mprCloseFile(file);
...
static MprFile *createFile(EjsMod *mp, char *name)
{
    ...
    if (file == 0) {
        mprLog("ejs doc", 0, "Cannot open %s", path);
        mp->errorCount++;
        return 0;
    }
    return file;
...
static void checkArgs(EjsMod *mp, Ejs *ejs, EjsName ownerName, EjsFunction *fun, EjsName qname, EjsDoc *doc)
{
    ...
    for (i = 0; i < (int) fun->numArgs; i++) {
        ...
        if (param == 0) { 
            if (mp->warnOnError) {
                mprLog("ejs", 0, "Missing documentation for parameter \"%@\" in function \"%@\" in type \"%@\"", 
    ...
    if (doc == 0 || doc->brief == 0) {
        if (mp->warnOnError) {
            if (!ejsIsType(ejs, fun)) {
                ...
                if (mp->warnOnError) {
                    mprLog("ejs", 0, "Missing documentation for \"%@.%@\"", fp->ownerName.name, qname.name);
                }
            }
        }
        return;
    ...
    if (doc) {
        ...
        if (count > 0) {
            ...
            for (next = 0; (param = mprGetNextItem(doc->params, &next)) != 0; ) {
                ...
                if (i < 0) {
                    mprLog("ejs doc", 0, "Bad @param reference for \"%s\" in function \"%@\" in type \"%@\"", param->key, 
...
static wchar *mergeDuplicates(Ejs *ejs, EjsMod *mp, EjsName qname, EjsDoc *doc, wchar *spec)
{
    ...
    while ((next = mcontains(next, "@duplicate")) != 0) {
        ...
        if ((dup = getDuplicateDoc(ejs, duplicate)) == 0) {
            mprLog("ejs doc", 0, "Cannot find @duplicate directive %s for %s", duplicate, qname.name);
        } else {
...
static EjsDoc *crackDoc(EjsMod *mp, EjsDoc *doc, EjsName qname)
{
    ...
    while (next) {
        ...
        if (match(token, "duplicate")) {
            ...
            if ((dup = getDuplicateDoc(ejs, duplicate)) == 0) {
                mprLog("ejs doc", 0, "Cannot find @duplicate directive %s for %@", duplicate, qname.name);
            } else {
                crackDoc(mp, dup, WEN(duplicate));
                mprCopyListContents(doc->params, dup->params);
                mprCopyListContents(doc->options, dup->options);
...
static void out(EjsMod *mp, char *fmt, ...)
{
    ...
    if (mp->file == 0) {
        ...
        if (mp->file == 0) {
            mprLog("ejs doc", 0, "Cannot open %s", mp->path);
            mp->errorCount++;
            return;
        }
    }
...
static EjsDoc *getDuplicateDoc(Ejs *ejs, wchar *duplicate)
{
    ...
    if (doc) {
        if (doc->docString == NULL || doc->docString->value[0] == '\0') {
            mprLog("ejs doc", 0, "Duplicate entry \"%s\" provides no description", duplicate);
            return 0;
        }
    }
    return doc;
