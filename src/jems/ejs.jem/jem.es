...
/*
    Background
        - Jem Repository contents are a hg repository
    ...
    - Think through catalog
        ...
        - Remote ops should download catalog every time
            install
            search
    ...
    - Jem Commands
        ...
        - jem orphans     # print a list of possibly redundant jems (rename stale)
        - jem rebuild
...
/*
    Store the best available information available about a jem
 */
class Jem {
    ...
    var local: Path             //  Path to physical local location (if known)
    var remote: String          //  Remote repository location
    var version: String         //  Actual version of the jem (if known)
    var min: String             //  Minimum acceptable version (if seaching)
    var max: String             //  Maximum acceptable version (if seaching)
    var package: Object         //  Package description file

    /*
     *  Create a new jem object. The name may be a path, bare jem name or a versioned jem name
     */
    function Jem(pathname: String, min: Number = 0, max: Number = MAX_VER) {
        pathname = Path(pathname).trimExt().toString()
...
class JemCmd 
{
    ...
    private const logFile: String = "stdout"
    private const logLevel: Number = 1
    ...
    private var defaultConfig = {
        ...
        directories: {
            ...
            //  TODO - is this used?
            ejs: Path("XXXXX"),
            jems: Path("/Users/mob/hg/ejs/jlocal"),
            modules: Path("/Users/mob/hg/ejs/modules"),
            lib: Path("/Users/mob/hg/ejs/lib"),
            bin: Path("/Users/mob/hg/ejs/bin"),
            repositories: [ Path("/Users/mob/hg/ejs/jremote") ],
        },
        filenames: {
...
*/
    ...
    /*
        Build a jem file
        @param path Path to the directory containing the jem. 
     */
    ...
    function edit(path: Path): Void {
        ...
        path.join(filenames.package).write(serialize(package))
    }
...
*/
    ...
    function publish(jem: Jem): Void {
        ...
        if (repo.contains("http://")) {
            ...
            makeDir(dir)
            let from = jem.local.join(jem.vername + ext.jem)
            let target = dir.join(jem.vername + ext.jem)
            cp(from, target)
            publishLocal()
        }
    }
    ...
    private function setdeps() {
        ...
        filenames.package.write(serialize(package))
    }
