...
module ejs.db.mysql {
    ...
    "ejs.db" class Mysql {
         ...
         */
        native "ejs.db" function Mysql(options: Object)
        ...
        function getTables(): Array {
            let cmd: String = "SELECT name from mysql_master WHERE type = 'table' order by NAME;"
            let grid: Array = query(cmd)
            let result: Array = new Array
            for each (let row: Object in grid) {
                let name: String = row["name"]
                if (!name.contains("mysql_") && !name.contains("_Ejs")) {
