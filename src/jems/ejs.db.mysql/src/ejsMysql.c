...
static EjsVar *dbConstructor(Ejs *ejs, EjsDb *db, int argc, EjsVar **argv)
{
     ...
     */
    db->arena = mprAllocArena(ejs, "sqlite", EJS_MAX_SQLITE_MEM, 0, 0);
    if (db->arena == 0) {
        return 0;
    }
     ...
     */
    if ((dbp = mprAllocWithDestructor(ejs, 1, (MprDestructor) dbDestructor)) == 0) {
...
//  sqlite3_exec(sdb, "PRAGMA count_changes = OFF", NULL, NULL, NULL);
    ...
    ejsSetProperty(ejs, (EjsVar*) db, ES_ejs_db_Database__name, 
        (EjsVar*) ejsCreateStringAndFree(ejs, mprGetPathBase(ejs, path)));
#endif
    return 0;
}
...
int ejs_db_mysql_Init(MprCtx ctx)
{
    return ejsAddNativeModule(ctx, "ejs.db.mysql", configureEventsTypes, _ES_CHECKSUM_ejs_db_mysql, EJS_LOADER_ETERNAL);
}

#else
void __dummy_ejsMysql() {}
#endif /* BLD_FEATURE_MYSQL */
