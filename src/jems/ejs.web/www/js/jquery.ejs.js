...
//////////////////////////////////
        ...
        callRemote: function () {
            var el     = this,
                ...
                //  MOB -- remove data-url when tree is not using it
                url    = el.attr('action') || el.attr('href') || el.attr('data-click') || el.attr('data-url') || el.attr('data-remote');
        ...
        click: function () {
            var el     = this,
                method = el.attr('method') || el.attr('data-method') || 'GET',
                url    = el.attr('action') || el.attr('href') || el.attr('data-click');

            if (url === undefined) {
                throw "No URL specified for click";
            }
            window.location = url;
        }
    });
...
//  MOB -- don't put id on the URL
    ...
    //  Ensure that click on submit buttons get added if remote
    ...
    $('a[data-method]:not([data-remote])').live('click', function (e){
        var link = $(this),
            ...
            token = $('meta[name=__ejs-security-token__]').attr('content'),

        if (token) {
          metadata_input += '<input name="__ejs-security-token__" value="' + token + '" type="hidden" />';
          form.hide()
              .append(metadata_input)
              .appendTo('body')
              .submit();
        } else {
          throw "Missing Security Token";
        }
        e.preventDefault();
    });

    //  MOB -- need to make this generic
    $('[data-click]').live('click', function (e) {
        $(this).click();
        e.preventDefault();
    });

    //  MOB -- data refresh code needed.
...
/////////////////////////////////////////
    ...
    //  MOB - put code here to extract data-refresh elements
