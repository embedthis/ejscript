...
/************************************************************************/
/*
 *  Start of file "../src/handlers/proxyHandler.c"
 */
/************************************************************************/

/* 
    proxyHandler.c -- Proxy 
...
static void openProxy(HttpQueue *q)
{
    ...
    proxy = mprCreateObjZeroed(q, Proxy);
    q->queueData = proxy;

    sock = mprCreateSocket(q);
    if (mprOpenClientSocket(sock, uri->host, uri->port) < 0) {
    }
    proxy->targetConn = createConn(conn->arena, uri->host, sock, ip, uri->port, NULL);
    maCreatePipeline(proxy->targetConn);
}
...
{
    httpSendPacketToNext(q->pair);
    httpSendPacketToNext(proxy->clientQ[HTTP_QUEUE_SEND]);
}
...
static void runProxy(HttpQueue *q)
{
}


static int parseProxy(Http *http, cchar *key, char *value, MaConfigState *state)
{
    HttpLocation    *location;
    HttpServer      *server;
    MaHost          *host;
    ...
    if (mprStrcmpAnyCase(key, "Action") == 0) {
        ...
        /*
            Create an alias and location with a proxyHandler and pathInfo processing
         */
        path = maMakePath(host, path);
        ...
        if ((location = maLookupLocation(host, prefix)) == 0) {
            location = maCreateLocation(http, state->location);
        ...
        httpSetHandler(http, host, location);
        return 1;
    }
    return 0;
}
...
/*
    Dynamic module initialization
 */
MprModule *maProxyHandlerInit(Http *http, cchar *path)
{
    MprModule   *module;
    HttpStage     *handler;

    module = mprCreateModule(http, "proxyHandler", BLD_VERSION, NULL, NULL, NULL);
    if (module == 0) {
        return 0;
    }

    handler = httpCreateHandler(http, "proxyHandler", HTTP_STAGE_ALL);
    if (handler == 0) {
        mprFree(module);
        return 0;
    }
    ...
    handler->outgoingData = outgoingProxyData;
    handler->incomingData = incomingProxyData; 
    handler->parse = parseProxy; 
    return module;
}
...
/************************************************************************/
/*
 *  End of file "../src/handlers/proxyHandler.c"
 */
/************************************************************************/
