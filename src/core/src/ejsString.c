...
static EjsString *stringConstructor(Ejs *ejs, EjsString *sp, int argc, EjsObj **argv)
{
    ...
    assure(argc == 0 || (argc == 1 && ejsIs(ejs, argv[0], Array)));
    sp->length = 0;
...
static EjsString *trimString(Ejs *ejs, EjsString *sp, int argc,  EjsObj **argv)
{
    ...
    assure(argc == 0 || (argc == 1 && ejsIs(ejs, argv[0], String)));
...
{
    assure(sp != head);
    assure(sp->next == NULL || sp->next == sp);
    assure(sp->prev == NULL || sp->next == sp);
