...
export class Uri {
    ...
    private _scheme: string | null = null
        ...
        if (typeof uri === 'object') {
            ...
            if (this._scheme && this._host) {
                try {
                    ...
                    if (this._port) urlStr += `:${this._port}`
                    urlStr += this._path || '/'
                    if (this._query) urlStr += `?${this._query}`
                    if (this._reference) urlStr += `#${this._reference}`
     ...
     */
    get components(): { scheme: string | null; host: string | null; port: number | null; path: string; query: string | null; reference: string | null } {
        return {
            scheme: this._scheme,
            host: this._host,
            port: this._port,
     ...
     */
    absolute(base: Uri | string | null = null): Uri {
        if (this.isAbsolute) {
     ...
     */
    complete(missing: Uri | string | null = null): Uri {
     ...
     * @returns Relative URI
     */
    relativeTo(origin: Uri | string | null = null): Uri {
        if (!origin) {
     ...
     */
    joinExt(ext: string, force: boolean = false): Uri {
        if (!ext.startsWith('.')) {
            ext = '.' + ext
        }
