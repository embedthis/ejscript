...
export class ByteArray extends Uint8Array {
     ...
     */
    constructor(size: number = 1024, growable: boolean = true) {
        super(size)
    ...
    static fromData(data: string | ArrayLike<number> | ArrayBufferLike, _encoding?: string): ByteArray {
        if (typeof data === 'string') {
            ...
            // Generic ArrayLike
            const length = (data as ArrayLike<number>).length
            const ba = new ByteArray(length, false)
            for (let i = 0; i < length; i++) {
                ba[i] = (data as ArrayLike<number>)[i]
            }
            ba._writePosition = length
            return ba
        }
     ...
     */
    private grow(amount: number): ByteArray {
        if (!this._growable) {
    ...
    readString(count: number = -1, encoding?: string): string | null {
        ...
        count = Math.min(count, this.available)
        const decoder = new TextDecoder((encoding || this._encoding) as any)
    ...
    readLong(): bigint | null {
        if (this.available < 8) return null
